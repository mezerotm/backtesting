...
def get_report_metadata(report_dir):
            ...
            with open(metadata_path, 'r', encoding='utf-8') as f:
                ...
                if 'start_date' in metadata:
                    metadata['start_date'] = datetime.strptime(
                        metadata['start_date'], '%Y-%m-%d').strftime('%Y-%m-%d')
                if 'end_date' in metadata:
                    metadata['end_date'] = datetime.strptime(
                        metadata['end_date'], '%Y-%m-%d').strftime('%Y-%m-%d')
                if 'created' in metadata:
                    metadata['created'] = datetime.strptime(
                        metadata['created'], '%Y-%m-%d %H:%M:%S').strftime('%Y-%m-%d %H:%M:%S')
    ...
    # If we couldn't read the metadata, create a basic one from the directory
    # name
...
def format_report_date(report):
    ...
    if report.get('timeframe') == "snapshot" or report.get(
            'type') == "financial":
        # Just get the date part
        return report.get('created', '-').split(' ')[0]
...
class DashboardUpdateHandler(FileSystemEventHandler):
    ...
    def _handle_event(self, event):
        ...
        if not event.src_path.endswith(
                '.html') or 'dashboard.html' in event.src_path:
            return
            ...
            def do_GET(self):
                if self.path == '/reports-data':
                    try:
                        ...
                        # Extract metadata from each report directory
                        reports = [
                            get_report_metadata(dir) for dir in report_dirs]

                        # Sort reports by creation time, newest first
                        reports = sorted(
                            reports, key=lambda report: report.get(
                                'created_timestamp', 0), reverse=True)
            ...
            def do_POST(self):
                if self.path == '/clean-results':
                    try:
                        ...
                        if os.path.exists(results_dir):
                            ...
                            for item in os.listdir(results_dir):
                                ...
                                try:
                                    ...
                                    print(
                                        f"Error while deleting {item_path}: {e}")
                        ...
                        self.end_headers()
                        self.wfile.write(json.dumps(
                            {'message': 'Results cleaned successfully'}).encode())
                    except Exception as e:
                        self.send_response(500)
                        self.send_header('Content-type', 'application/json')
                        self.end_headers()
                    ...
                    try:
                        ...
                        # Verify the path is within results directory and
                        # exists
                        if os.path.commonprefix([os.path.abspath(
                                report_path), results_dir]) == results_dir and os.path.exists(report_path):
                            ...
                            self.end_headers()
                            self.wfile.write(json.dumps(
                                {'message': 'Report deleted successfully'}).encode())
                        else:
...
def start_dashboard_server(open_browser=True):
    ...
    generate_dashboard(refresh_interval=refresh_interval)
...
if __name__ == "__main__":
    # Start FastAPI app (server/api/dashboard.py) with uvicorn in a background
    # process
    fastapi_proc = subprocess.Popen([sys.executable,
                                     "-m",
                                     "uvicorn",
                                     "server.api.dashboard:router",
                                     "--host",
                                     "0.0.0.0",
                                     "--port",
                                     "8001",
