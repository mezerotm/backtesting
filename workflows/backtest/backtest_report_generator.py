...
def get_value_class(metric, value):
    ...
    if "Return" in metric and "Drawdown" not in metric:
...
def create_backtest_report(
    ...
    ) and not disable_ai_explanations
    ...
    # Define the metrics we want to display - make sure they match the keys in
    # the Stats object
    metrics = [
        'Return [%]',
        'Buy & Hold Return [%]',
    ...
    symbol = args.symbol if hasattr(
        args, 'symbol') else args.ticker if hasattr(
        args, 'ticker') else "Unknown"
    ...
    start_date = args.start_date if hasattr(
        args, 'start_date') else args.start if hasattr(
        args, 'start') else "Unknown"

    # Get end_date from args, handling both end_date and end naming
    end_date = args.end_date if hasattr(
        args, 'end_date') else args.end if hasattr(
        args, 'end') else datetime.now().strftime("%Y-%m-%d")

    # Ensure end_date is not "None" or None
    if end_date is None or end_date == "None":
        end_date = datetime.now().strftime("%Y-%m-%d")

    # Get initial_capital from args, handling both initial_capital and cash
    # naming
    initial_capital = args.initial_capital if hasattr(
        args, 'initial_capital') else args.cash if hasattr(
        args, 'cash') else 10000
    ...
    if not valid_strategies:
        ...
        if strategies and len(strategies) > 0:
            # Use the first requested strategy as a fallback with default
            # values
