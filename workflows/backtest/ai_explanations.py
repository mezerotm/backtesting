...
class AIExplainer:
    ...
    def __init__(self):
        ...
        if not self.api_key:
            print(
                "Warning: No OpenAI API key found in config. AI explanations will not be available.")
        else:
            ...
            if validate_api_key(self.api_key):
                ...
                print(
                    "API key validation failed. AI explanations will not work correctly.")
    ...
    def explain_metric(self,
        ...
        try:
            ...
            prompt = self._create_prompt(
                metric_name, metric_value, strategy_name, metric_context)
    ...
    def explain_strategy_overview(
            self, strategy_name: str, metrics: Dict[str, Any]) -> str:
        ...
        try:
            ...
            prompt = f"""
            Analyze the following backtest results for the trading strategy '{
                strategy_name}':
    ...
    def _create_prompt(self,
        ...
        if metric_context:
            context_text = "\n".join(
                [f"{k}: {v}" for k, v in metric_context.items() if k != metric_name])
