...
class StrategyUtils:
    ...
    @staticmethod
    def add_complete_bollinger_bands(
            strategy,
            close_prices,
            period=20,
            num_std_dev=2):
    ...
    def add_trade_markers(strategy, trades=None):
        ...
        if trades:
            ...
            for trade in trades:
                ...
                if hasattr(
                        trade,
                        'entry_bar') and trade.entry_bar < len(buy_signals):
                    buy_signals[trade.entry_bar] = strategy.data.Close[trade.entry_bar]

                if hasattr(trade, 'exit_bar') and trade.exit_bar < len(
                        sell_signals):
                    ...
                    if hasattr(trade, 'pl_pct'):
                        marker = {
                            ...
                            'value': 0,  # Baseline
                            # Scale marker size with profit/loss
                            'size': abs(trade.pl_pct) * 0.5 + 5,
            ...
            results['trade_count'] = strategy.I(
                # Fixed position for marker placement
                lambda: np.zeros(len(strategy.data)) + 0.5,
    ...
    def setup_complete_chart(strategy, data):
        ...
        performance_results = StrategyUtils.add_performance_metrics(
            strategy, data)
