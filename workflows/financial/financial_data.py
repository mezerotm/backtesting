...
class FinancialDataFetcher(BaseFetcher):
    ...
    def calculate_peg_ratio(
            self,
            market_cap: float,
            annual_financials: pd.DataFrame) -> Optional[float]:
        ...
        try:
            ...
            pe_ratio = market_cap / (latest_earnings * 4)
    ...
    def fetch_financial_statements(self, symbol: str, years: int) -> Dict:
        ...
        try:
            ...
            quarterly = self._fetch_polygon_financials(
                symbol, "quarterly", years * 4)
                ...
                try:
                    ...
                    if hasattr(fin, 'financials'):
                        ...
                        if hasattr(fin.financials, 'income_statement'):
                            ...
                            record['revenue'] = self._get_value_from_datapoint(
                                getattr(inc, 'revenues', None))
                            record['gross_profit'] = self._get_value_from_datapoint(
                                getattr(inc, 'gross_profit', None))
                            record['operating_income'] = self._get_value_from_datapoint(
                                getattr(inc, 'operating_income_loss', None))
                            record['net_income'] = self._get_value_from_datapoint(
                                getattr(inc, 'net_income_loss', None))
                        # Balance Sheet
                        if hasattr(fin.financials, 'balance_sheet'):
                            bs = fin.financials.balance_sheet
                            record['total_assets'] = self._get_value_from_datapoint(
                                getattr(bs, 'assets', None))
                            record['current_assets'] = self._get_value_from_datapoint(
                                getattr(bs, 'current_assets', None))
                            record['current_liabilities'] = self._get_value_from_datapoint(
                                getattr(bs, 'current_liabilities', None))
                            record['inventory'] = self._get_value_from_datapoint(
                                getattr(bs, 'inventory', None))
                            record['liabilities'] = self._get_value_from_datapoint(
                                getattr(bs, 'liabilities', None))
                        # Cash Flow Statement
                        if hasattr(fin.financials, 'cash_flow_statement'):
                            cf = fin.financials.cash_flow_statement
                            record['operating_cash_flow'] = self._get_value_from_datapoint(
                                getattr(cf, 'net_cash_flow_from_operating_activities', None))
                            record['capital_expenditure'] = self._get_value_from_datapoint(
                                getattr(cf, 'net_cash_flow_from_investing_activities', None))
                            record['financing_cash_flow'] = self._get_value_from_datapoint(
    ...
    def _get_value_from_datapoint(self, datapoint) -> float:
        ...
        if hasattr(datapoint, 'value'):
            return float(
                datapoint.value) if datapoint.value is not None else 0.0
        try:
    ...
    def fetch_key_metrics(self, symbol: str) -> Dict:
        ...
        try:
            ...
            print(f"[DEBUG] Fetched float (share_class_shares_outstanding) for {
                  symbol}: {getattr(details, 'share_class_shares_outstanding', None)}")
            ...
            annual_financials = financials.get(
                'annual_financials', pd.DataFrame())
            ...
            if not sector or sector == 'N/A':
                for df in [
                        financials.get('quarterly_financials'),
                        financials.get('annual_financials')]:
                    if isinstance(df, pd.DataFrame) and not df.empty:
                        sector = df['sector'].dropna(
                        ).iloc[0] if 'sector' in df.columns and df['sector'].notna().any() else sector
                        break
            if not sector:
                sector = 'N/A'
            float_val = getattr(
                details, 'share_class_shares_outstanding', None)
            print(f"[DEBUG] float_val used in fundamentals for {
                  symbol}: {float_val}")
            peg_ratio = None
            if market_cap and not annual_financials.empty:
                peg_ratio = self.calculate_peg_ratio(
                    market_cap, annual_financials)
            fundamentals = {
                'market_cap': market_cap,
                'weighted_shares': getattr(
                    details,
            ...
            print(f"[ERROR] Error fetching key metrics for {symbol}")
            import traceback
