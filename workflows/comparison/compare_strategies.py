...
def compare_strategies(data, strategies, cash=10000, commission=0.002):
    ...
    for name, strategy_class in strategies.items():
        ...
        stats = bt.run()
        # Store the stats object directly instead of trying to access
        # _trade_data
        results[name] = stats
...
def plot_strategy_comparison(results_df, show_plot=True, save_path=None):
    ...
    # Import matplotlib.pyplot here to avoid conflicts
    import matplotlib.pyplot as plt
    ...
    # Only try to show the plot if explicitly requested and we're in an
...
def run_example():
    ...
    save_interactive_comparison(
        results, os.path.join(
            results_dir, 'strategy_comparison.html'))
