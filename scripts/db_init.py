...
- The Python path is set up to allow imports from server/
- If you get "ModuleNotFoundError: No module named 'server'", make sure you're running from the project root
- Use: python scripts/db_init.py or make db-init from the project root
"""

# CRITICAL: Add the project root to Python path BEFORE any imports
...
def verify_project_structure():
    ...
    if not os.path.exists(server_dir) or not os.path.exists(config_dir):
        logger.error(f"""
ERROR: Incorrect directory structure detected!
Current directory: {current_dir}
Expected to find: server/ and config/ directories

SOLUTION: Make sure you're running this script from the project root directory.
Run: cd /path/to/backtesting && python scripts/db_init.py
