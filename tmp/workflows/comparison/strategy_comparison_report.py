...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Create a consolidated report from CSV results.')
...
def create_strategy_ranking(results):
    ...
    strategy_ranking = strategy_ranking.sort_values(
...
def visualize_best_strategies(results, output_path=None):
    ...
    # Set non-interactive backend if saving to file in non-interactive
    # environment
...
def create_interactive_scenario_comparison(results, output_path=None):
    """
    Create an interactive HTML visualization for scenario comparison.
    ...
    fig = make_subplots(
        ...
        cols=1,
        subplot_titles=[
            "Strategy Performance by Sharpe Ratio",
            "Strategy Performance by Return [%]"],
        vertical_spacing=0.15)

    # Create color map for strategies
    import plotly.express as px
    colors = px.colors.qualitative.Plotly
    strategy_colors = {strategy: colors[i % len(
        colors)] for i, strategy in enumerate(all_strategies)}
...
def generate_html_report(
    ...
    rel_interactive_path = os.path.relpath(
        interactive_path, os.path.dirname(output_file))
    html_content += f"""
        <div class="interactive-link">
            <a href="{rel_interactive_path}
        " target="_blank">View Interactive Strategy Comparison</a>
        </div>
    """
    ...
    html_content = html_content.replace(
        'BEST_STRATEGIES_IMG_REL_PATH', rel_img_path)
